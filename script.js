...
function startGUI () {
    ...
    let github = gui.add({ fun : () => {
        window.open('https://github.com/lakeheck/WebGL-Fluid-Simulation');
...
const splatVelShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    void main () {
        ...
        p.x *= aspectRatio;
        vec3 splat = vec3(0.0);
        splat = smoothstep(0.0, 1.0, normalize(texture2D(uDensityMap, vUv).xyz)) * (normalize(texture2D(uForceMap, vUv).rgb)*2.0-1.0) * uVelocityScale;
        splat.z = 0.0;
        vec3 base = texture2D(uTarget, vUv).xyz;
        gl_FragColor = vec4(base + splat, 1.0);
    }
